        ...
        <div class="w-full px-8 md:w-3/4 md:px-0 mx-auto md:py-24 relative z-10">
            <div class="flex justify-center flex-col items-center">
                ...
                <div class="flex md:flex-row justify-center items-center w-full gap-10">
                    ...
                    <div class="flex flex-col xl:flex-row gap-10 mt-10 w-full md:mt-20">
                        <div class="w-full xl:w-9/12">
                            @if($bimbel['banner_type'] == 'video')
                                <div class="aspect-w-16 aspect-h-9">
                                    <iframe src="{{ str_replace('watch?v=', 'embed/', $bimbel['banner_video']) }}"
                            ...
                            @else
                                <img src="{{ Storage::disk('public')->exists($bimbel['banner_image']) ? asset('storage/' . $bimbel['banner_image']) : asset('images/assets/img.png') }}"
                                    alt="Samoedra Bimbel"
                                    class="rounded-3xl h-96 w-full object-cover">
                            @endif
                            <!-- Kelebihan Bimbel -->
                            <h1 class="font-semibold text-[#3E5467] text-3xl xl:text-4xl mt-8"
                                style="font-family: 'Fredoka';">{{ $bimbel['benefit_title'] ?? 'Kelebihan Bimbel Kami' }}</h1>
                            <p class="text-[#A2A2BD] mt-4" style="font-family: 'Onest';">
                                {{ $bimbel['benefit_description'] ?? '' }}
                            </p>
                        </div>
                            ...
                            <div class="bg-[#F3EEE6] p-8 rounded-3xl">
                                ...
                                <div class="space-y-3">
                                    <div
                                        ...
                                        <p class="text-[#A2A2BD] text-end max-w-72" style="font-family: 'Onest';">{{ $bimbel['age_range'] ?? '0-Dewasa' }}</p>
                                    </div>
                                    <div
                                        class="flex justify-between border-b-2 border-dashed pb-3 border-[#E8A26A] items-center">
                                        <flex class="flex-row flex items-center space-x-3">
                                        ...
                                        <p class="text-[#A2A2BD] text-end max-w-72" style="font-family: 'Onest';">{{ $bimbel['operating_hours'] ?? '9.00-20.00 (Pilihan waktu dapat disesuaikan)' }}</p>
                                    </div>
                                    <div
                                        class="flex justify-between border-b-2 border-dashed pb-3 border-[#E8A26A] items-center">
                                        <flex class="flex-row flex items-center space-x-3">
                                        ...
                                        <p class="text-[#A2A2BD] text-end max-w-72" style="font-family: 'Onest';">{{ $bimbel['operating_days'] ?? 'Senin-Sabtu' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <section id="informasi " class="w-full">
                        ...
                        <div class="w-full lg:w-1/2 flex flex-col ">
                            <div class="space-y-3">
                                ...
                                <h1 class="text-[#3E5467] font-semibold text-4xl xl:text-5xl text-start"
                                    style="font-family: 'Fredoka';">{{ $bimbel['program_title'] ?? 'Program Bimbel Rumah Samoedra' }}
                                </h1>
                                <p class="text-[#A2A2BD] max-w-xl mt-5" style="font-family: 'Onest';">{{ $bimbel['program_description'] ?? '' }}</p>
                            </div>
                            <div class="space-y-4 mt-6">
                                @if(isset($bimbel['program_points']))
                                    @foreach($bimbel['program_points'] as $point)
                        ...
                        <div class="w-full lg:w-1/2 relative flex justify-center items-center">
                            <div class="w-full max-w-[500px] h-[500px] p-3 xl:p-5 bg-[#EFF5F6] mask-faq relative z-10">
                                <div class="mask-faq h-full">
                                    <img src="{{ Storage::disk('public')->exists($bimbel['program_image']) ? asset('storage/' . $bimbel['program_image']) : asset('images/assets/img_detail_layanan.png') }}"
                    ...
                    <div
                        ...
                        <div class="flex flex-row gap-7 py-10 overflow-x-auto">
                            @php
                                ...
                                $bimbelData = json_decode(file_get_contents($jsonPath), true);
                                $facilities = $bimbelData['facilities'] ?? [];
                            @endphp

                            @foreach($facilities as $facility)
                            <div class="w-80 h-64 rounded-4xl bg-white p-6 flex flex-col relative">
                                <img src="{{ Storage::disk('public')->exists($facility['image']) ? asset('storage/' . $facility['image']) : asset('images/assets/img_layanan.png') }}"
                                    alt="{{ $facility['title'] }}"
                                    class="w-full h-44 rounded-3xl object-cover">
                                <h1 class="text-3xl text-[#3E5467] font-semibold text-center mt-3"
                                    style="font-family: 'Fredoka';">{{ $facility['title'] }}</h1>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <table class="w-full border-collapse border border-[#E8A26A] min-w-[800px]">
                        ...
                        <tbody style="font-family: 'Onest'; color: #3E5467;">
                            @php
                                $jsonPath = storage_path('app/public/bimbel/data.json');
                                $bimbelData = json_decode(file_get_contents($jsonPath), true);
                                $pricingItems = $bimbelData['pricing_items'] ?? [];
                            @endphp

                            @foreach($pricingItems as $index => $item)
                            <tr>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center">{{ $index + 1 }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3">{{ $item['service'] }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center">{{ $item['levels'] }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center">{{ $item['registration_promo'] }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center line-through">Rp {{ number_format($item['price_per_session'], 0, ',', '.') }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center">Rp {{ number_format($item['price_per_session_promo'], 0, ',', '.') }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center line-through">Rp {{ number_format($item['monthly_price'], 0, ',', '.') }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center">Rp {{ number_format($item['monthly_price_promo'], 0, ',', '.') }}</td>
                                <td class="border border-[#E8A26A] px-4 py-3 text-center">{{ $item['sessions_per_month'] }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
