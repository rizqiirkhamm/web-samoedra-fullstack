...
<body>
    ...
    <div class="flex justify-center flex-col items-center ">
        <h1 class="text-[#3E5467] mt-16 font-semibold text-4xl xl:text-5xl text-center z-50"
            style="font-family: 'Fredoka';">
            Halo👋🏻, Selamat Datang <br> Di Daycare Rumah Samoedra
        </h1>
        <div class="flex gap-5 mt-10 z-50">
            <a href="{{ route('program') }}" class="border-[1.5px]  rounded-full bg-[#3E5467] flex items-center gap-2 text-white px-4 py-3 transition-all duration-300 ">
                ...
                <div class="w-full xl:w-9/12">
                    ...
                    <div class="rounded-3xl overflow-hidden h-96">
                        <iframe width="100%" height="100%"
                            src="{{ str_replace('watch?v=', 'embed/', $daycareData['banner_video']) }}" frameborder="0"
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                            allowfullscreen class="w-full h-full object-cover"></iframe>
                    </div>
                    @else
                    <img src="{{
                        (strpos($daycareData['banner_image'], 'storage/') === 0) ? asset($daycareData['banner_image']) :
                        ((strpos($daycareData['banner_image'], 'images/') === 0) ? asset($daycareData['banner_image']) :
                        (Storage::disk('public')->exists($daycareData['banner_image']) ? asset('storage/' . $daycareData['banner_image']) : asset('images/daycare/banner.jpg')))
                    }}"
                        alt="Samoedra Daycare" class="rounded-3xl h-96 w-full object-cover">
                    @endif
                    <!-- Kelebihan Daycare -->
        ...
        <div class="w-full  px-8 md:w-3/4 md:px-0 mx-auto flex flex-col pb-16 md:flex-row mt-24">
            <div class="w-full lg:w-1/2 flex flex-col justify-center">
                <div class="space-y-3">
                    ...
                    <p class="text-[#A2A2BD] max-w-xl mt-5" style="font-family: 'Onest';">Layanan penitipan anak
                        berkualitas dengan pendekatan pendidikan yang holistik dan fokus pada perkembangan anak</p>
                </div>
                <div class="space-y-4 mt-6">
                    ...
                    <div class="flex flex-row space-x-3">
                        ...
                        <h1 class="text-[#3E5467] font-semibold text-xl" style="font-family: 'Fredoka';">{{ $point }}
                        </h1>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="w-full lg:w-1/2 relative flex justify-center items-center">
                <div class="w-full max-w-[500px] h-[500px] p-3 xl:p-5 bg-[#EFF5F6] mask-faq relative z-10">
                    <div class="mask-faq h-full">
                        <img src="{{
                            (strpos($daycareData['program']['image'], 'storage/') === 0) ? asset($daycareData['program']['image']) :
                            ((strpos($daycareData['program']['image'], 'images/') === 0) ? asset($daycareData['program']['image']) :
                            (Storage::disk('public')->exists($daycareData['program']['image']) ? asset('storage/' . $daycareData['program']['image']) : asset('images/daycare/default-program.jpg')))
                        }}"
                            class="w-full h-full object-cover object-center" alt="Program Daycare">
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex flex-row gap-7 py-10 overflow-x-auto">
                        ...
                        <div class="w-80 h-64 rounded-4xl bg-white p-6 flex flex-col relative">
                            <img src="{{
                                (strpos($facility['image'], 'storage/') === 0) ? asset($facility['image']) :
                                ((strpos($facility['image'], 'images/') === 0) ? asset($facility['image']) :
                                (Storage::disk('public')->exists($facility['image']) ? asset('storage/' . $facility['image']) : asset('images/daycare/default-facility.jpg')))
                            }}"
                                alt="{{ $facility['title'] ?? 'Fasilitas Daycare' }}"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="border border-[#E8A26A] px-4 py-3 text-[#3E5467] text-center"
                                ...
                                {{ $daycareData['about_daycare']['details']['usia'] ?? '6 bln s/d 12 thn' }}</td>
                            <td class="border border-[#E8A26A] px-4 py-3 text-[#3E5467] text-center"
                                style="font-family: 'Onest';">{{ $price['description'] ?? '-' }}</td>
                            <td class="border border-[#E8A26A] px-4 py-3 text-[#3E5467] text-center"
                                style="font-family: 'Onest';">{{ $price['price'] }}</td>
                            <td class="border border-[#E8A26A] px-4 py-3 text-[#3E5467] text-center"
                                style="font-family: 'Onest';">{{ $price['food_cost'] ?? '12.5k / Porsi, 5k Snack' }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
